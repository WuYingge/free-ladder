...
from factors.meta_factor import (
    ...
    CompositeRankFactor,
    ConditionSpec,  # 加权 rank 合成
    ConditionalFactor,
    MultiConditionalFactor,    # 条件因子 (if condition > threshold, keep signal else 0)
    NegateFactor,         # 取负
...
pr_20_1_filter = ConditionalFactor(
    signal=pr_20,
    condition=TrendR2Factor(window=120, output="r2"),
    op="gt", threshold=0.5,
)
...
GROUPS: list[tuple] = [
    ...
    # )),
    (f"c2_clean_120_above_{rf.exclude_above_pct}_below_{rf.exclude_below_pct}", c2, NO_FILTERS, (rf,)) for rf in RANK_FILTERS_120
]
...
# ── 报告标题 ──
TITLE: str = "宽动量基线回测 — c2_clean_120"

# ── 输出目录名称 tag（替换默认的 "{n}g"）──
BASENAME_TAG: str = "c2_clean_120_start_date_sensitive"
...
START_DATES: tuple[str, ...] | None = ("2020-01-01", "2020-01-02", "2020-01-03", "2020-01-04", "2020-01-05", "2020-01-06", "2020-01-07", "2020-01-08")
# 示例: ("2020-01-01", "2021-01-01", "2022-01-01")
END_DATES:   tuple[str, ...] | None = ("2026-07-10", "2026-07-11", "2026-07-12", "2026-07-13", "2026-07-14", "2026-07-15", "2026-07-16", "2026-07-17")
# 示例: ("2024-12-31", "2025-12-31", "2026-05-29")

# ── 标的池：显式使用相关性去重后的合并版（158 只唯一 ETF）──
# 直接读取合并版 CSV（data/const/etf_index_map_corr_merged.csv），
# 不依赖 ETF_INDEX_MAP 的环境变量切换，避免误跑成 489 只的名称去重池。
_REPO_ROOT = Path(__file__).resolve().parents[3]
_DEDUP_POOL_CSV = _REPO_ROOT / "data" / "const" / "etf_index_map_corr_merged.csv"
SYMBOLS: tuple[str, ...] | None = tuple(
    pd.read_csv(_DEDUP_POOL_CSV, dtype=str, encoding="utf-8-sig")["selected_symbol"].str.strip()
)
