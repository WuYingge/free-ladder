...
def _generate_md_report(
    ...
    if quality_results:
        ...
        if ac is not None and not ac.empty:
            lag1 = ac.iloc[0]["mean_autocorr"]
            lag20 = ac.iloc[-1]["mean_autocorr"]
            buf.write("### 1.4 自相关衰减\n\n")
            buf.write("| 滞后 (天) | 均值自相关 | 标准差 |\n|---|---|---|\n")
            for _, row in ac.iterrows():
                buf.write(f"| {int(row['lag'])} | {row['mean_autocorr']:.4f} | {row['std_autocorr']:.4f} |\n")
            buf.write("\n")
            buf.write(f"![自相关衰减](autocorr_decay.png)\n\n")
            buf.write("> **怎么算的**: 对每个标的，计算 factor(t) 和 factor(t−lag) 的 Spearman 秩相关系数，取所有标的的截面均值。\n")
            buf.write(f"> **代表什么**: lag1={lag1:.2f}，说明今天的因子值和昨天{'几乎一样' if lag1 > 0.9 else '差别很大' if lag1 < 0.3 else '有一定相似性'}（自相关={lag1:.0%}）。")
            if abs(lag20) > 0.3:
    ...
    if predictive_results:
        ...
        rank = predictive_results.get("rank_ic", {}).get("summary", {})
        pearson = predictive_results.get("pearson_ic", {}).get("summary", {})
        ...
        if rank:
            ...
            buf.write("### 2.1-2.2 IC（信息系数）\n\n")
            buf.write("| 指标 | Rank IC | Pearson IC |\n|---|---|---|\n")
            for key, label in [("mean", "均值"), ("std", "标准差"), ("ir", "IR (mean/std)"), ("t_stat", "t 统计量"), ("pos_ratio", "IC>0 比例"), ("n_days", "有效天数")]:
                rv = rank.get(key, float("nan"))
                pv = pearson.get(key, float("nan"))
                buf.write(f"| {label} | {rv:.6f} | {pv:.6f} |\n")
            ...
            buf.write("> **代表什么**: ")
            if not np.isnan(mean_ic):
                buf.write(f"Rank IC 均值 = {mean_ic:.4f}，")
                if mean_ic > 0.02:
            ...
            buf.write("\n")
            buf.write(f"> **Rank IC vs Pearson IC**: {'Rank IC ≈ Pearson IC，说明因子收益不是由少数极端值驱动的，分布较均匀。' if abs(mean_ic - pearson.get('mean', float('nan'))) < 0.02 else 'Rank IC 和 Pearson IC 差异较大——注意是否有肥尾效应影响线性评估。'}\n")
            ...
            buf.write(f"![滚动 IC 时序](rolling_ic.png)\n\n")
    ...
    if grouping_results:
        ...
        buf.write(f"> **本层定位**: 从「因子能否排序」深化到「按因子分组能赚多少钱」。将 IC 的统计显著性转化为可交易的经济显著性。\n\n")

        qs = grouping_results.get("quantile_summary")
        if qs is not None and not qs.empty:
            # 找出 top 和 bottom 组
            q_labels = qs.index.tolist()
            bottom_label = q_labels[0]
            top_label = q_labels[-1]
            bottom_ret = qs.loc[bottom_label, "mean_return"] if "mean_return" in qs.columns else float("nan")
            top_ret = qs.loc[top_label, "mean_return"] if "mean_return" in qs.columns else float("nan")

            buf.write("### 3.1 分位组收益\n\n")
            buf.write(f"> 每天把 {panel.n_symbols} 个标的按因子值从低到高分成 {n_q} 组，")
            buf.write(f"{bottom_label} = 因子值最低的 {100//n_q}%（过去表现最差的），{top_label} = 因子值最高的 {100//n_q}%（过去表现最好的）。每组等权持有，计算未来收益的均值。\n\n")

            buf.write("| 分位组 | 平均收益 | 标准差 | 胜率 |\n|---|---|---|---|\n")
            for idx, row in qs.iterrows():
                mr = row.get("mean_return", float("nan"))
                wr = row.get("win_rate", float("nan"))
                buf.write(f"| {idx} | {mr:.6f} | {row.get('std_return', float('nan')):.6f} | {wr:.2%} |\n")
            buf.write("\n")

            # 检查是否有单调性
            returns_list = [qs.loc[l, "mean_return"] for l in q_labels if l in qs.index and "mean_return" in qs.columns]
            top_bottom_diff = top_ret - bottom_ret if not np.isnan(top_ret) and not np.isnan(bottom_ret) else float("nan")

            buf.write(f"![分位组收益柱状图](quantile_returns_bar.png)\n\n")
            buf.write(f"![分位组累计收益](quantile_cumret.png)\n\n")

            buf.write("> **代表什么**: ")
            if not np.isnan(top_bottom_diff):
                if top_bottom_diff > 0.001:
                    buf.write(f"Top 组（{top_label}）的收益（{top_ret:.4%}）明显高于 Bottom 组（{bottom_label}）的收益（{bottom_ret:.4%}），差值为 {top_bottom_diff:.4%}。因子排序能力较好。\n")
                elif top_bottom_diff < -0.001:
        ...
        # ── Long-Short ──
        ls = grouping_results.get("longshort", {})
        if ls:
            ann_ret = ls.get("annualised_return", float("nan"))
            sharpe = ls.get("sharpe")
            mdd = ls.get("max_drawdown", float("nan"))
            cum_ret = ls.get("cumulative_return", float("nan"))

            buf.write("### 3.2 Long-Short 多空组合\n\n")
            buf.write("| 指标 | 值 |\n|---|---|\n")
            buf.write(f"| 年化收益 | {ann_ret:.4%} |\n")
            buf.write(f"| 年化波动 | {ls.get('annualised_volatility', float('nan')):.4%} |\n")
            buf.write(f"| Sharpe | {sharpe if sharpe is not None else 'N/A'} |\n")
            buf.write(f"| 最大回撤 | {mdd:.4%} |\n")
            buf.write(f"| 累计收益 | {cum_ret:.4%} |\n")
            buf.write("\n")
            buf.write(f"![多空累计收益](longshort_cumret.png)\n\n")
            buf.write("> **怎么算的**: 做多 Top 组 + 做空 Bottom 组，计算这个多空组合的每日收益序列，再算年化收益/波动/Sharpe/最大回撤。\n")
            buf.write("> **代表什么**: 多空收益是因子**纯 alpha 的最直接度量**——剥离了市场整体涨跌（beta），只看因子排序本身能不能赚钱。\n")
            if not np.isnan(ann_ret):
                if ann_ret > 0.05:
                    buf.write(f"这个因子的多空年化收益为 +{ann_ret:.1%}，说明按因子排序做多最强 + 做空最弱是能赚钱的。\n")
                elif ann_ret < -0.05:
                    buf.write(f"这个因子的多空年化收益为 {ann_ret:.1%}（负），说明目前的排序方向是亏钱的——可能需要反向使用（做多低因子值 + 做空高因子值）。\n")
                else:
                    buf.write(f"这个因子的多空年化收益接近零（{ann_ret:.1%}），纯 alpha 很弱。\n")
            if sharpe is not None:
                buf.write(f"Sharpe = {sharpe:.2f}，{'收益风险比不错' if sharpe > 0.5 else '收益风险比偏低' if sharpe > 0 else '负收益'}。\n")
            buf.write(f"最大回撤 = {mdd:.1%}（{'风险很高' if mdd > 0.5 else '风险中等' if mdd > 0.2 else '风险较低'}）。\n\n")
        ...
        if mono:
            ...
            buf.write("### 3.4 单调性检验\n\n")
            buf.write(f"- 严格单调成立比例: {strict:.2%}\n")
            buf.write(f"- 宽松单调成立比例: {loose:.2%}\n")
            buf.write(f"- 单调方向: {direction}\n")
            jt_stat = mono.get("jt_statistic")
            jt_p = mono.get("jt_pvalue")
            if jt_stat is not None:
                buf.write(f"- JT 统计量: {jt_stat:.4f}\n")
                buf.write(f"- JT p 值: {jt_p:.6f}\n")
            buf.write("\n")
            buf.write("> **怎么算的**: 对每天截面检查 Q1 > Q2 > ... > Qn 是否成立。严格单调 = 每个相邻组都满足大小关系。JT (Jonckheere-Terpstra) 检验评估是否存在统计显著的趋势。\n")
            buf.write(f"> **代表什么**: 严格单调成立仅 {strict:.1%}——{'说明因子排序在不同分位组之间的一致性很强' if strict > 0.5 else '大多数交易日分组收益的排序并不完美，因子在中间组上的区分度有限' if strict > 0.1 else '因子排序非常不稳定，几乎每天的组间收益顺序都不一样'}。非单调的因子在极端值有效但中间不可靠，做分桶筛选时会有坑。\n\n")
...
def _write_summary_line(
    ...
    if predictive_results:
        ...
        if not np.isnan(mean_ic):
            if mean_ic > 0.05:
                points.append(f"✅ Rank IC = {mean_ic:.4f}（靠谱），IR = {ir:.3f}")
            elif mean_ic > 0.02:
                points.append(f"Rank IC = {mean_ic:.4f}（可用但偏弱），IR = {ir:.3f}")
            elif mean_ic > -0.02:
                points.append(f"⚠️ Rank IC = {mean_ic:.4f}（接近零，预测力很弱），IR = {ir:.3f}")
            else:
                points.append(f"🔴 Rank IC = {mean_ic:.4f}（负值，方向与预期相反），IR = {ir:.3f}")
    ...
    if grouping_results:
        ...
        if not np.isnan(ann_ret):
            if ann_ret > 0.10:
                points.append(f"✅ Long-Short 年化收益 = {ann_ret:.1%}（优秀）")
            elif ann_ret > 0.03:
                points.append(f"Long-Short 年化收益 = {ann_ret:.1%}（中等）")
            elif ann_ret > -0.03:
                points.append(f"⚠️ Long-Short 年化收益 ≈ 零（{ann_ret:.1%}），分组后的 top/bottom 差异太小")
            else:
                points.append(f"🔴 Long-Short 年化收益 = {ann_ret:.1%}（负值）")
